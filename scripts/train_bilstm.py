...
def train():
    ...
    # Loss weights — tuned to prioritize accuracy since intra/inter are easily passing
    w_center = 1.0        # Decreased to focus more on CE
    w_repulsion = 0.5     # Decreased as inter-class is already negative
    w_ce = 5.0            # Increased significantly to push accuracy > 92%
    ...
    encoder = get_encoder("cnn_bilstm", input_dim=input_dim, embedding_dim=embed_dim)
    encoder.to(DEVICE)
    ...
    for epoch in range(epochs):
        ...
        if kpi_score > best_kpi_score + 0.01:
            ...
            torch.save(checkpoint, CHECKPOINTS_DIR / "best_encoder_bilstm.pt")
    ...
    best_ckpt = torch.load(CHECKPOINTS_DIR / "best_encoder_bilstm.pt", map_location=DEVICE, weights_only=False)
