"""
Generalization Training — Train on 3 classes, withhold xr_ar completely.

This demonstrates the model's ability to generalize to unseen traffic types.
The encoder learns a general embedding space from gaming, streaming, and voip,
then at eval time, xr_ar is classified using few-shot k-NN in that space.

KPI Target: ≥85% accuracy on unseen xr_ar with 5-shot support set.
"""
...
# The class to withhold (unseen class for generalization test)
# The class to withhold (unseen class for generalization test)
HELD_OUT_CLASS = 3  # xr_ar
HELD_OUT_NAME = "xr_ar"
...
def train():
    ...
    logger.info("=" * 70)
    logger.info("🧪 GENERALIZATION TRAINING: 3 classes (withholding xr_ar)")
    logger.info(f"  Held-out class: {HELD_OUT_NAME} (label {HELD_OUT_CLASS})")
    logger.info(f"  Embedding dim: {embed_dim}")
    logger.info(f"  Device: {DEVICE}")
    logger.info("=" * 70)

    # Load data and REMOVE the held-out class
    X_train_full, y_train_full = load_split(str(SPLITS_DIR / "train.csv"))
    X_val_full, y_val_full = load_split(str(SPLITS_DIR / "val.csv"))

    # Filter out xr_ar
    train_mask = y_train_full != HELD_OUT_CLASS
    val_mask = y_val_full != HELD_OUT_CLASS

    X_train = X_train_full[train_mask]
    y_train_orig = y_train_full[train_mask]
    X_val = X_val_full[val_mask]
    y_val_orig = y_val_full[val_mask]

    # Remap labels to be contiguous (0, 1, 2) for 3-class training
    # Original: gaming=0, streaming=1, voip=2, xr_ar=3
    # After removing streaming(1): gaming=0, voip=2, xr_ar=3 → remap to 0, 1, 2
    label_map = {}
    for new_idx, old_label in enumerate(sorted(np.unique(y_train_orig))):
        label_map[old_label] = new_idx
    y_train = np.array([label_map[l] for l in y_train_orig])
    y_val = np.array([label_map[l] for l in y_val_orig])
    logger.info(f"  Label mapping: {label_map}")

    input_dim = X_train.shape[1]
    num_classes = len(np.unique(y_train))  # Should be 3

    logger.info(f"  Train: {X_train.shape} (removed {(~train_mask).sum()} {HELD_OUT_NAME} samples)")
    logger.info(f"  Val: {X_val.shape} (removed {(~val_mask).sum()} {HELD_OUT_NAME} samples)")
    logger.info(f"  Classes: {num_classes} (gaming, streaming, voip)")
    logger.info(f"  Class dist: {dict(zip(*np.unique(y_train, return_counts=True)))}")
    ...
    for epoch in range(epochs):
        ...
        kpi_score = (
            ...
            max(1.0 - inter, 0) * 1.5 +
            min(val_acc, 1.0) * 2.0 +
            (10.0 if all_pass else 0.0)
        )

        elapsed = time.time() - t0
        train_loss = total_loss / max(n_batches, 1)

        if epoch % 20 == 0 or kpi_score > best_kpi_score:
            logger.info(
        ...
        if kpi_score > best_kpi_score + 0.01:
            ...
            torch.save(checkpoint, CHECKPOINTS_DIR / "generalization_encoder.pt")
            logger.info(f"  💾 Saved generalization model (KPI={kpi_score:.2f})")
        else:
            patience_counter += 1
    ...
    # Generate 3-class training embeddings for few-shot eval
    best_ckpt = torch.load(CHECKPOINTS_DIR / "generalization_encoder.pt", map_location=DEVICE, weights_only=False)
    ...
    np.save(EMBEDDINGS_DIR / "gen_train_embeddings.npy", train_embeddings)
    np.save(EMBEDDINGS_DIR / "gen_train_labels.npy", y_train)

    logger.info("✅ Generalization training complete! Now run eval_generalization.py")
